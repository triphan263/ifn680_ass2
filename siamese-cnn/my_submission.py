...
def generate_warped_dataset(dataset):
    ...
    for i in range(hard_pair_records):
        ...
        h_x_train[i] = assign2_utils.random_deform(x_train[index], #[index]/255,
    ...
    for i in range(int(dataset['hard_pair'] * test_records)):
        ...
        h_x_test[i] = assign2_utils.random_deform(x_test[index], # x_test[index]/255,
                             dataset['hard_pair_rotation'], 
                             dataset['hard_pair_variation'])
    ...
    for i in range(easy_pair_records):
        index = np.random.randint(0, x_train.shape[0])
        e_x_train[i] = assign2_utils.random_deform(x_train[index], # x_train[index]/255,
    ...
    for i in range(int((1 - dataset['hard_pair']) * test_records)):
        index = np.random.randint(0, x_test.shape[0])
        e_x_test[i] = assign2_utils.random_deform(x_test[index], # x_test[index]/255,
                             dataset['easy_pair_rotation'], 
                             dataset['easy_pair_variation'])
        e_y_test[i] = y_test[i]
...
SETTINGS = {
    'dataset_generation': {
        ...
        'warped_dataset_records': 60000,   # number of records in warped dataset (D dataset)
    ...
    'network_configs': { # experiment on original dataset
        'epochs': 1,
        'original_dataset': True            # original_dataset = True. Using original dataset
